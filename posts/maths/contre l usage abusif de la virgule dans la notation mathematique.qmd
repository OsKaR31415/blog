...
description: "Pourquoi la virgule devrait être moins utilisée dans la notation mathématique."
date: 2023-07-23
categories: [ maths ]
...
La notation mathématique traditionnelle cherche à traduire des phrases et des calculs en symboles.
...
La notation mathématique intéressante principalement pour deux choses :
 ...
 - elle est plus lisible : on préfère $\forall x \in N, \forall i, j \in \mathbb{N}, \quad i\times j|n \implies (i\mid n) \wedge (j\mid n)$ plutôt que "_si un entier est divisible par le produit de deux entiers, alors il est divisible par chacun des deux entiers_"
...
On voit parfois aussi $\lbrace x \in E : \mathscr{P}(x) \rbrace$.
...
J'ai pourtant déjà vu cette expression : $\{ \overrightarrow{MN}, M, N \in F\}$.
...
Ces remplacements ont plusieurs avantages :
 - ils retirent toute ambiguïté, même celles qui n'en sont pas, mais qui sont des erreurs de lecture
...
Mon avis personnel est donc :
 ...
 - Dans un contexte où le but est simplement de faire comprendre (solution ou correction d'un exercice, cours et énoncés simples de choses connues ou évidentes), utiliser des virgules (et autres abréviations) peut être avantageux
     ...
     - cela permet d'écrire plus vite (notamment utile lorsqu'on résout un exercice)
J'ajouterai que, avant d'utiliser des notations raccourcies, il est nécessaire de bien comprendre comment les notations formelles fonctionnent. Pour cela, écrire sans aucune abréviation (de manière rigoureuse) est utile.
Il vaut mieux s'habituer d'abord à une utilisation stricte, puis utiliser des raccourcis quand c'est justifié (ceci ne se limite pas aux raccourcis de notation traités ici, mais est valable pour toutes les abréviations, comme $2\mathbb{N}$ pour $\{ 2n \mid n \in \mathbb{N} \}$, ou bien $\sum\limits_{i}v_{i}$ pour $\sum\limits_{i=0}^n v_{i} \text{ où } n \text{ est la dimension du vecteur } v$)
